...
export async function POST(req) {
  ...
  if (!address || !signature) return new Response(JSON.stringify({ ok: false, error: "missing" }), { status: 400 });

  await dbConnect();
  const user = await User.findOne({ walletAddress: address.toLowerCase() });
  if (!user) return new Response(JSON.stringify({ ok: false, error: "user not found" }), { status: 404 });
  ...
  try {
    ...
    // signature valid: create/update user, assign new nonce
    ...
    res.headers.append("Set-Cookie", cookie.serialize("token", token, {
      httpOnly: true,
      path: "/",
